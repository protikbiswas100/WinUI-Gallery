...
.EXAMPLE
    .\New-WinAppSdkSample.ps1 -Version 2.3.2-experimentalA -Launch

.EXAMPLE
    # Auto-detect the latest monthly experimental release and smoke-test it:
...
if (-not $Version) {
    ...
    $Version = $exp[-1]   # flatcontainer returns versions in ascending SemVer order
}
...
if (-not $WindowsSdkPackageVersion) {
    ...
    $WindowsSdkPackageVersion = $stable[-1]
}
